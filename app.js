{
  ...
  const fillOrRemoveChars = (counter, loopLength, baseStr, blanks, domEle, speed, endStr) => {
    ...
    .join(`${String.fromCharCode(160)}<wbr>`)}
      ...
      setTimeout(() => fillOrRemoveChars(counter, loopLength, baseStr, blanks, domEle, speed, endStr), speed) // Calls itself again after speed ms pause
    }
  }
  ...
  const generateStr = (endStr, domEle, speed, dispTime, loopLength, baseStr = ``) => {
    ...
    setTimeout(() => fillOrRemoveChars(0, loopLength, baseStr, blanks, domEle, speed), (speed * loopLength) + dispTime) // timer based on how long it takes the fill animation to fill in every non-space character and wait a to display them
  }
  ...
  // Invoked as soon as the page loads
  ...
  // Loops the animation after that first invocation of bannerGimmick
  setInterval(() => bannerGimmick(banner, 50, 2000, 500,
    `Spencer Whitehead`,
    `a Full Stack Software Engineer`,
    `someone who loves programming`,
    `always learning and developing my skills`), 20000) // Tragically, this delay between plays of the animation runner is hardcoded
